...
function getArg(name) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (
      arg === "--fullscreen" &&
      ["borderless", "bordered"].includes(nextArg)
    ) {
      fullscreen = nextArg;
...
const getNextClient = (clients, activeWindow, direction) => {
  ...
  const nextClientIndex =
    (currentClientIndex + direction + sameClassClientsSorted.length) %
    sameClassClientsSorted.length;
...
const toggleWrongFullscreenClient = (clients, nextClient) => {
  const wrongFullscreenClient = sameWorkspaceClients(clients, nextClient).find(
    (client) => {
  ...
  if (wrongFullscreenClient) {
    exec(
      `hyprctl dispatch focuswindow address:${wrongFullscreenClient.address}`
    );
